...
import com.asialjim.microapplet.mams.chl.wx.core.context.BaseWeChatApiRes;
import com.asialjim.microapplet.mams.chl.wx.core.token.WeChatAccessTokenRepository;
    ...
    private boolean isAccessTokenInvalid(CachedClientHttpResponse response) {
        try {
            ...
            boolean json = Optional.ofNullable(contentType)
                    .map(item -> MediaType.APPLICATION_JSON.includes(item) || item.toString().contains("+json"))
                    .orElse(false);
