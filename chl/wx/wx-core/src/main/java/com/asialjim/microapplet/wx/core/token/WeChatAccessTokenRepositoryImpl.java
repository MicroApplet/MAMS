...
import com.asialjim.microapplet.wx.core.client.WeChatAccessTokenClient;
...
import com.asialjim.microapplet.wx.core.context.WeChatAccessTokenRes;
import com.asialjim.microapplet.wx.core.context.WeChatApiException;
...
//    private WeChatApplicationRepository weChatApplicationRepository;
    @Resource
    private ObjectProvider<WeChatApplicationRepository> weChatApplicationRepositoryObjectProvider;
    @Resource
    private WeChatAccessTokenCache weChatAccessTokenCache;
    @Resource
    ...
    public String accessToken(String appidOrIndex) {
        WeChatApplication app = this.weChatApplicationRepositoryObjectProvider.getObject().resolve(appidOrIndex);
        //weChatApplicationRepository.resolve(appidOrIndex);
    ...
    public String refreshAccessToken(String appidOrIndex) {
        WeChatApplication app = this.weChatApplicationRepositoryObjectProvider.getObject().resolve(appidOrIndex);
        //WeChatApplication app = weChatApplicationRepository.resolve(appidOrIndex);
        this.weChatAccessTokenCache.remove(app.getAppid());
        return obtainAndCache(app);
    }

    private String accessToken(WeChatApplication app) {
        String appid = app.getAppid();
        String accessToken = this.weChatAccessTokenCache.get(appid);
        if (StringUtils.isNotBlank(accessToken)) return accessToken;

        Lock lock = this.weChatAccessTokenCache.lock(appid);
        lock.lock();
        try {
            accessToken = this.weChatAccessTokenCache.get(appid);
            if (StringUtils.isNotBlank(accessToken)) return accessToken;
            return obtainAndCache(app);
        } finally {
            lock.unlock();
    ...
    private String obtainAndCache(WeChatApplication app) {
        ...
        this.weChatAccessTokenCache.set(app.getAppid(), res.getAccessToken());
