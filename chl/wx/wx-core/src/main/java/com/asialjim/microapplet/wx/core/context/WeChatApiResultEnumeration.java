...
public enum WeChatApiResultEnumeration {
    ...
    CODE_40028(40028, "Invalid model for custom menu", "不合法的自定义菜单使用用户"),
    ...
    CODE_65302(65302, "There is no corresponding model.", "没有相应的用户"),
    ...
    CODE_9001004(9001004, "Failed to obtain shaking peripheral model information.", "获取摇周边用户信息失败"),
    ...
    public void thr(String... errs){
        Integer errcode = this.getErrcode();
        String errmsg = this.getErrmsg();
        String cnMsg = this.getCnMsg();
        List<String> list = new ArrayList<>(Arrays.asList(errs));
        list.addAll(List.of(String.valueOf(errcode),errmsg,cnMsg));
        WeChatResCode.ApiErr.thr(list);
    }
}